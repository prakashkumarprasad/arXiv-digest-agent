...
- `retrieval.py::_broaden_attempt_1` — the category-removal regex (`\s+cat:\w+`) did not match `cat:cs.CL` inside parentheses and stopped at the dot, and AND→OR then turned the category clause into `... OR (cat:cs.CL OR.LG)`, which matched every recent cs.CL/cs.LG paper. Real symptom: gibberish topic queries "succeeded" after one broadening and digested the newest unrelated paper with exit code 0 → the whole `AND (cat:...)` clause is now removed before AND→OR. Found by a real CLI run, not by the unit tests.
- `retrieval.py::_broaden_attempt_2` — worked on raw tokens of attempt 1's output and produced malformed queries (`all:(all:x OR all:all:y`, HTTP 400 from arXiv) → terms are now extracted with `_extract_terms` and the two longest are kept (stand-in for highest-IDF). This resolves the earlier "KNOWN, NOT FIXED" note about `all:all:`. Verified on real arXiv: a gibberish topic query now ends after 3 searches with ZERO_RESULTS_EXHAUSTED and exit code 1.
- KNOWN, NOT FIXED: `select_paper` has no relevance floor, so a topic query where one word matches something (e.g. "zzqxv nonexistent flibbertigibbet") still digests the newest paper containing that word.
- `services/llm.py` — provider rate limits (HTTP 429) were treated like bad JSON and retried immediately with the error text in the prompt → the client now waits the time the provider reports (Retry-After header, else "try again in Ns") and re-sends the same request; waits over 60s fail fast. Real symptom: an `ask` right after a `digest` on Groq's 8000 TPM free tier.

- `retrieval.py`, `state.py` — the exhausted zero-result message named only the last broadened query as "Original query" and gave no next step (spec §5.3) → every query tried is recorded in a new `queries_tried` state key (declared in `AgentState`, since undeclared keys are silently dropped) and the ZERO_RESULTS_EXHAUSTED detail lists them and suggests fewer or different keywords or an arXiv ID; failed-attempt warnings now include the query.
- `nodes/selection.py`, `graph.py` — `select_paper` had no relevance floor: recency and full-text points (0.25*10 + 0.15*10 = 4 of 10) let any recent paper win, so a topic query where one word matched (e.g. "zzqxv nonexistent flibbertigibbet") digested the newest unrelated paper → candidates below `MIN_LLM_RELEVANCE` (4/10, judged against the user's own request and applied before ranking) are rejected; if none remain the node returns `paper=None` and a NO_RELEVANT_PAPER error, and a new conditional edge (`_route_after_selection`) ends the run instead of calling fetch_pdf. The floor is skipped if LLM scoring itself fails. Supersedes the earlier "KNOWN, NOT FIXED" note about the missing floor. The threshold is not calibrated beyond manual checks.
- KNOWN, NOT FIXED: re-running `digest` on a topic thread that already has a briefing leaves the old `briefing`/`paper` in the checkpoint if the new run fails early; reset those keys in cli.py if it matters.
- KNOWN, NOT FIXED: after each failed broadening the graph re-runs `search_arxiv` with the same broadened query (5 arXiv calls for 2 attempts).
