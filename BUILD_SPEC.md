...
### Stage 8b
- `tests/conftest.py` (new) — `isolated_settings` autouse fixture points `CHROMA_PERSIST_DIR`, `SQLITE_DB_PATH` and `PDF_CACHE_DIR` at `tmp_path` via `monkeypatch.setenv` and calls `reset_settings()`, so tests never touch the real `.data/` (`Settings` creates directories on instantiation).
- `tests/conftest.py` — `_block_network` autouse fixture makes `socket.socket.connect` refuse non-localhost addresses and sets `HF_HUB_OFFLINE=1`, so unit tests cannot reach the network.
- `tests/conftest.py` — `sentence_transformers` is replaced in `sys.modules` at import time (a bare assignment, never undone) to avoid its ~11 s first import, and `_mock_embedding_model` patches `chunker._get_embedding_model` and `vectorstore._get_client`. Consequence: unit tests can never exercise the real embedding model; anything that needs real embeddings (abstain calibration) has to run outside pytest.
- Test files at the end of the stage: `test_query_understanding.py` (ID/URL detection, LLM fallback), `test_chunker.py` (section boundaries, overlap, deterministic ids), `test_parser_fallback.py` (a corrupt PDF degrades to `degraded_abstract_only`), `test_selection_zero_results.py` (broaden runs at most twice, routing), `test_qa_abstain.py`, `test_multiturn_history.py` (message counts 2, 4, 6; `_start` returns `{}`), `test_citations.py` (`[Sn]` to `chunk_id` mapping). Later stages added `test_qa_paths.py`, `test_zero_results_graph.py`, `test_broaden_queries.py`, `test_broaden_message.py`, `test_relevance_floor.py`, `test_llm_rate_limit.py` and `test_query_rewrite.py` (70 tests in total, all offline).
- SUPERSEDED — the first version of `test_selection_zero_results.py` replaced the graph-level zero-result test with routing-function tests plus a hand-merged state simulation, because `graph.invoke` hit the recursion limit, and its log entry claimed the simulation "proves the end-to-end flow terminates". It did not: the recursion limit was a real bug (`retries` missing from `AgentState`, see "Stage 8 post-verification fixes") and the workaround hid it. The rule for this situation was to mark the test `xfail` and log a `Not fixed —` entry.
- Validation method — later tests were checked by reintroducing the bug each one guards against and confirming a specific test failed. Two tests turned out to be blind and were fixed (see "Stage 8 post-verification fixes").

### Stage 8c
- `docs/architecture.md` — the first version marked `candidates`, `sections` and `retrieved` as `operator.add` reducers (only `errors`, `warnings` and `messages` are), lacked `retries` and `queries_tried`, and claimed all failure paths were verified through real runs and unit tests without evidence → rewritten. The Mermaid block is now inserted by a script from `build_graph().get_graph().draw_mermaid()` and must be regenerated whenever `graph.py` changes (the `select_paper` → END edge made the first diagram stale; check: the generated text is a substring of the doc).
- `examples/briefing_2401_12345.json`, `examples/sample_session.md` — the first versions came from an Ollama run; regenerated from a real Groq run starting from an empty session store (`.data/sessions.sqlite` deleted first) by a capture script that refuses to publish output containing a traceback, a failed QA attempt or a missing citation. Briefing text still varies between runs (one captured briefing attributed a Wiener-DL vs Wiener comparison to Kernel-DL).
- `examples/` — removed `briefing_2201_12345.json` and `briefing_2301_12345.json` (generated before the `meta` fix, so `meta` was empty). Kept `briefing_1706_03762.json` and `briefing_2609_07966.json` (Ollama runs; the second is a topic search).
- Real CLI runs verified: `digest` completes; `ask` shows only `start` and `qa_node` (no re-parse); the off-topic question returns `That isn't covered in this paper.`; `sessions` lists the thread.
...
- `LLM_PROVIDER=groq` — not verified by the coding agent (no key in its environment), later verified with the user's own Groq key: a full digest and Q&A capture (`examples/sample_session.md`) and a fresh-clone `make install` / `make test` / `make demo` (see "Stage 8e"). The README provider table lists only what was actually run.

### Stage 8e
- `.gitattributes` — created with `* text=auto`. After `git checkout` on Windows, working copies can end up with CRLF line endings, so any script that edits source files must normalize line endings first.
- `scripts/check_abstain.py` — moved from the repo root and rewritten by the coding agent with mocks. With `query_chunks` mocked, its "distance calibration confirms 0.45" output is circular: it is a logic check (message counts, abstain routing), not a calibration. The real evidence for `ABSTAIN_MAX_DISTANCE=0.45` is the Stage 7 entry: two papers, in-paper maxima 0.378 and 0.413 against off-topic minima 0.494 and 0.488. Still only two papers.
- `.env.example` — lists every variable read in `config.py`, including `ABSTAIN_MAX_DISTANCE`; the default `LLM_PROVIDER` is now `groq` (the README's documented path) with the key left blank.
- Root scratch scripts were removed from the repo root; the ones kept live in `scripts/`.
- `docs/demo.gif` — the coding agent had created a 6-byte placeholder file → deleted; the README links no GIF (the assignment accepts a pasted example run, and the recorded video is a separate deliverable).
- `Brain.md` — an agent-generated codebase inventory that contradicted the README (it still said Groq was unverified) → untracked and added to `.gitignore`.
- Secret check — `.env` is not tracked; no Groq/Google/`sk-` key patterns in tracked files; `git log --all -p | grep -cE "gsk_[A-Za-z0-9]{20,}"` and the same count for `AIza`/`sk-` patterns are 0. The only history hits for `gsk_` are this file's own text describing the scan.
- Fresh-clone check — the first run FAILED in a clean venv (see "Stage 8 post-verification fixes", dependencies). After the fix, a `git clone`, `make install`, `make test` (70 passed) and `make demo` with `LLM_PROVIDER=groq` all succeeded: the demo printed the briefing, a cited answer and the abstain line.

### Stage 8 post-verification fixes (found by real runs, break-it checks and a fresh clone)
- `state.py` — `retries` was in the §3 state shape but missing from `AgentState`, so LangGraph silently dropped it from every node return: `broaden_query` re-ran "attempt 1" forever and the zero-result path hit `GraphRecursionError`; the `fetch_pdf` retry counter never persisted either → declared `retries: dict[str, int]` (no reducer: nodes return the full updated dict). Found by `tests/test_zero_results_graph.py`, which runs the compiled graph; the routing-function tests missed it. `cli.py` now passes `"retries": {}` on every digest, because the value persists in the checkpoint.
- `nodes/retrieval.py` — `_broaden_attempt_1`'s regex (`\s+cat:\w+`) did not match `cat:cs.CL` inside parentheses and stopped at the dot, and AND→OR then turned the category clause into `... OR (cat:cs.CL OR.LG)`, which matched every recent cs.CL/cs.LG paper; `_broaden_attempt_2` built malformed queries (`all:(all:x OR all:all:y`, HTTP 400 from arXiv). Real symptom: a nonsense topic query digested the newest paper on arXiv with exit code 0 → the whole `AND (cat:...)` clause is removed before AND→OR, and attempt 2 extracts terms with `_extract_terms` and keeps the two longest (a stand-in for "highest IDF"). Found by a real CLI run, not by the unit tests. Evidence after the fix: "zzqxv qwzxq flibberzzq" makes three searches and ends with `ZERO_RESULTS_EXHAUSTED`, exit code 1.
- `nodes/retrieval.py`, `state.py` — the exhausted message named only the last broadened query as "Original query" and gave no next step (spec §5.3) → every query tried is recorded in a new `queries_tried` state key (declared in `AgentState`) and the message lists them and suggests fewer or different keywords or an arXiv ID.
- `nodes/selection.py`, `graph.py` — `select_paper` had no relevance floor: recency and full-text points (0.25×10 + 0.15×10 = 4 of 10) let any recent paper win → candidates below `MIN_LLM_RELEVANCE` (4/10, judged against the user's own request and applied before ranking) are rejected; if none remain the node returns `paper=None` and a `NO_RELEVANT_PAPER` error, and a conditional edge (`_route_after_selection`) ends the run instead of calling `fetch_pdf`. The floor is skipped if LLM scoring itself fails. Evidence: "sourdough bread fermentation recipes" found 15 papers, the best scored 1/10, the run stopped after `select_paper` with exit code 1; "KV-cache compression for LLMs" digested a relevant paper. The threshold was set by hand.
- `services/llm.py` — provider rate limits (HTTP 429) were treated like bad JSON and retried at once with the error text in the prompt → the client waits the time the provider reports (Retry-After header, else "try again in Ns") and re-sends the same request, up to 4 times; waits over 60 s fail fast. Real symptom: an `ask` right after a `digest` on Groq's 8,000 tokens-per-minute free tier (limit 8000, requested 4284).
- `nodes/qa.py`, `services/prompts.py` — a follow-up such as "Which of them suppresses outliers best?" was rewritten into a query listing all eleven method names, retrieval pulled a results table to the top, and the answer named Wiener-DR (lowest MSE) instead of Kernel-DL → the rewrite prompt asks for a short noun phrase and forbids copying lists, history messages are clipped to 300 characters, and a rewrite that is empty or longer than 30 words (`MAX_REWRITE_WORDS`) is discarded in favour of the original question. After the fix the answer named Kernel-DL and cited the chunk that discusses outliers. Found by comparing the answer with the paper's own briefing.
- `tests/test_query_rewrite.py` — the word-limit test built its "overlong" input from the same constant as the code, so raising the limit could never fail it → the test now uses fixed word counts. An earlier break-it check had also shown that the abstain-path history spread and the reversed `[Sn]` map were not covered → `test_qa_paths.py` was added.
- `pyproject.toml`, `Makefile` — found by the fresh clone: a clean install lacked `langgraph-checkpoint-sqlite` (import error in `graph.py`, so every CLI command crashed), `openai` (the LLM client uses the `openai` SDK for both Groq and Ollama; the old `groq` extra installed a different SDK the code never imports) and `pytest`; `make install` also pulled in the unused `gemini` extra → declared the missing packages, replaced the `groq` extra with `dev = ["pytest"]`, and `make install` now runs `pip install -e ".[dev]"`. Only a fresh clone can find this: the developer machine had everything installed by hand.
- Resolved — the Stage 5 note about stopwords in `_build_arxiv_query` (a `STOPWORDS` set now filters terms).
- Known, not fixed — `--top-k`, `--auto` and `--model` are accepted but mostly ignored (Stage 8a); a pronoun follow-up right after an abstained turn is rewritten toward the off-topic subject; metadata questions ("what is the title") abstain; citation `page` is 0 on the pdfplumber path and `section` labels can be wrong; text from math-heavy PDFs can be garbled (missing spaces, `(cid:...)` tokens); the briefing synthesis can attach a number to the wrong method; after a failed broadening the same search runs again (five arXiv calls for two attempts); re-running `digest` on a topic thread that fails early leaves the old `briefing` and `paper` in the checkpoint; `graph.py` still has unused `_sqlite_checkpointer` globals; the Gemini provider is unverified; the abstain threshold was calibrated on two papers.

---

## 10. Definition of done

- [x] `git clone && make install && make test && make demo` works on a clean machine with a Groq key — evidence: a fresh clone in a new virtual environment installed, passed 70 tests and ran the demo (briefing, cited answer, abstain line). The Ollama path ran in earlier stages; local Ollama crashed once on Windows.
- [x] `pytest` passes, including failure-path tests — evidence: 70 tests, offline, about 2 s locally, covering a broken PDF, zero results end to end, an irrelevant match, the abstain gate, rate limits and the follow-up rewrite.
- [x] `examples/briefing_*.json` and `examples/sample_session.md` are committed from a real run — evidence: a real Groq session from an empty session store; two older Ollama briefings are kept as extra examples.
- [x] `docs/architecture.md` has a Mermaid graph generated from the compiled LangGraph — evidence: the text from `draw_mermaid()` is a substring of the doc (checked after the last graph change).
- [x] README has architecture, state table, setup, example run, failure handling, rate-limit note, Design Decisions & Tradeoffs, known limitations and "what I'd do next" — evidence: the example run is pasted from the real Groq session, and the provider table lists only what was run.
- [x] No secrets committed; `.env` is gitignored — evidence: the tracked-file and full-history scans in Stage 8e.
- [ ] Pushed to a public GitHub repository (manual).
- [ ] 4-minute video reflection recorded and linked (manual; deliverable 3 of the assignment).
